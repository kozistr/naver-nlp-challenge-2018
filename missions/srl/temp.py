...
def _rnn_layer(inputs, seq_len, hidden_size, dropout):
    def _build_cell(hidden_size, dropout):
        cell = tf.contrib.rnn.CoupledInputForgetGateLSTMCell(
            hidden_size,
            use_peepholes=True,
            initializer=tf.contrib.layers.variance_scaling_initializer(factor=3., mode='FAN_AVG', uniform=True),
            state_is_tuple=True)
    ...
    with tf.variable_scope("recurrent"):
        ...
        with tf.variable_scope("forward"):
            cell_fw = _build_cell(hidden_size, dropout)
            y_fw = _dynamic_rnn(cell_fw, x, seq_len, "forward")

        with tf.variable_scope("backward"):
            cell_bw = _build_cell(hidden_size, dropout)
            y_bw = _dynamic_rnn(cell_bw, x, seq_len, "backward")

        return y_fw + y_bw


def encoder(encoder_input, seq_len, hidden_size, num_hidden_layers, dropout_rate, scope=None):
...
seq = "att+"
if seq == "att":
    # attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(num_units=self.word_lstm_dim,
    #                                                            memory=outputs,
    #                                                            memory_sequence_length=self.word_lengths)
    attention_mechanism = tf.contrib.seq2seq.LuongAttention(num_units=self.word_lstm_dim,
                                                            memory=outputs,
                                                            memory_sequence_length=self.word_lengths)
    ...
    attn_cell = tf.contrib.seq2seq.AttentionWrapper(temp_cell(self.word_lstm_dim),
                                                    attention_mechanism,
                                                    attention_layer_size=self.word_lstm_dim)

    helper = tf.contrib.seq2seq.TrainingHelper(outputs, self.word_lengths)
    decoder = tf.contrib.seq2seq.BasicDecoder(
        cell=attn_cell,
        helper=helper,
        initial_state=attn_cell.zero_state(dtype=tf.float32, batch_size=self.batch_size).clone(cell_state=stat_x),
        output_layer=None,
    )

    outputs, _, output_lengths = tf.contrib.seq2seq.dynamic_decode(decoder,
