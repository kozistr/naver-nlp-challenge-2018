...
def bind_model(sess):
    ...
    def infer(input, **kwargs):
        ...
        for morph, ne_dict, character, seq_len, char_len, _, step in dataset.get_data_batch_size(
                parameter["batch_size"], train=False):
            feed_dict = {model.morph: morph,
                         model.ne_dict: ne_dict,
                         model.character: character,
                         model.sequence: seq_len,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=120 + 1, required=False, help='Epoch value')
    parser.add_argument('--batch_size', type=int, default=10, required=False, help='Batch size')  # 32
